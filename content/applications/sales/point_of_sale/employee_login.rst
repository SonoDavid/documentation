...
Access the multi-employee setting from the :ref:`POS settings <configuration/settings>` or the
:icon:`fa-ellipsis-v` (:guilabel:`vertical ellipsis`) button located on the top-right corner of a
POS card on the main POS dashboard. Then,

#. Activate the :guilabel:`Multi Employees per Session` feature.
#. Add the allowed employees with **basic POS functionalities** in the :guilabel:`Basic rights`
   field.
#. Add the allowed employees with **extended POS functionalities** in the :guilabel:`Advanced
   rights` field.
